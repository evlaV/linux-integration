...
static bool platform_enter_test_done;
...
int hibernation_platform_enter(void)
{
	...
	if (hibernation_test(TEST_HIBERNATE_PLATFORM_ENTER)) {
...
int hibernate(void)
{
	...
	if (in_suspend) {
		...
		swsusp_free();
		if (!error && !hibernation_test(TEST_HIBERNATE_IMAGE_WRITE)) {
			if (hibernation_mode == HIBERNATION_TEST_RESUME)
				snapshot_test = true;
			else
				error = power_down();
		}
		if (error || hibernation_test(TEST_HIBERNATE_IMAGE_WRITE) || platform_enter_test_done) {
