...
static const char *const cal_enable_text[] = {
	[CAL_UNKNOWN] = "unknown",
	[CAL_START] = "start",
	[CAL_STOP] = "stop",
};
...
static int hid_lgo_light_event(struct command_report *cmd_rep)
{
...
static int hid_lgo_device_status_event(struct command_report *cmd_rep)
{
	switch (cmd_rep->device_type) {
	case LEFT_CONTROLLER:
...
static int hid_lgo_raw_event(struct hid_device *hdev, struct hid_report *report,
			     u8 *data, int size)
{
	...
	case MCU_CONFIG_DATA:
		...
		case GET_DPI_CFG:
			...
			break;
		case GET_LIGHT_CFG:
			ret = hid_lgo_light_event(cmd_rep);
			break;
		case GET_DEVICE_STATUS:
			ret = hid_lgo_device_status_event(cmd_rep);
			break;
...
/* GET_VERSION_INDEX */
static ssize_t get_version_show(struct device *dev,
				struct device_attribute *attr, char *buf,
				enum version_data_index index,
...
#define ATTR_GO_VERSION_RO(_name, _attrname, _dtype)                          \
	static ssize_t _name##_show(struct device *dev,                       \
				    struct device_attribute *attr, char *buf) \
	{                                                                     \
		return get_version_show(dev, attr, buf, _name.index, _dtype); \
	}                                                                     \
...
#define ATTR_GO_DEVSTATUS_RO(_name, _attrname, _dtype, _ctype)                 \
	static ssize_t _name##_show(struct device *dev,                        \
				    struct device_attribute *attr, char *buf)  \
	{                                                                      \
		return device_status_show(dev, attr, buf, _name.index, _dtype, \
					  _ctype);                             \
	}                                                                      \
	DEVICE_ATTR_RO_NAMED(_name, _attrname)

/* SET_TRIGGER_CFG/SET_JOYSTICK_CFG/SET_GYRO_CFG */
				      ...
				      size_t count, enum dev_type device_type)
{
	...
	ret = sysfs_match_string(cal_enable_text, buf);
	if (ret < 0)
		return ret;
					...
					char *buf)
{
	...
	for (i = 1; i < ARRAY_SIZE(cal_enable_text); i++)
		count += sysfs_emit_at(buf, count, "%s ", cal_enable_text[i]);
...
#define ATTR_GO_CAL_WO(_name, _attrname, _cmd, _rtype, _dtype)                \
	static ssize_t _name##_store(struct device *dev,                      \
				     struct device_attribute *attr,           \
				     const char *buf, size_t count)           \
	{                                                                     \
		return calibrate_config_store(dev, attr, buf, _cmd,           \
...
static int rgb_attr_show(void)
{
	...
	return rgb_cfg_call(drvdata.hdev, GET_LIGHT_CFG, index, 0, 0);
};
				...
				size_t count)
{
	...
	ret = rgb_cfg_call(drvdata.hdev, SET_LIGHT_CFG, index, rgb_profile, 6);
	if (ret)
		return ret;
			      ...
			      const char *buf, size_t count)
{
	...
	ret = rgb_cfg_call(drvdata.hdev, SET_LIGHT_CFG, LIGHT_MODE_SEL, &val, 1);
	if (ret)
		return ret;
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = rgb_cfg_call(drvdata.hdev, GET_LIGHT_CFG, LIGHT_PROFILE_SEL, 0, 0);
	if (ret)
		return ret;
...
static void hid_lgo_brightness_set(struct led_classdev *led_cdev,
...
ATTR_GO_VERSION_RO(version_protocol_mcu, "protocol_version", USB_MCU);

struct go_cfg_attr version_firmware_mcu = { FIRMWARE_VERSION };
ATTR_GO_VERSION_RO(version_firmware_mcu, "firmware_version", USB_MCU);

struct go_cfg_attr version_hardware_mcu = { HARDWARE_VERSION };
ATTR_GO_VERSION_RO(version_hardware_mcu, "hardware_version", USB_MCU);
...
ATTR_GO_VERSION_RO(version_product_tx_dongle, "product_version", TX_DONGLE);

struct go_cfg_attr version_protocol_tx_dongle = { PROTOCOL_VERSION };
ATTR_GO_VERSION_RO(version_protocol_tx_dongle, "protocol_version", TX_DONGLE);

struct go_cfg_attr version_firmware_tx_dongle = { FIRMWARE_VERSION };
ATTR_GO_VERSION_RO(version_firmware_tx_dongle, "firmware_version", TX_DONGLE);

struct go_cfg_attr version_hardware_tx_dongle = { HARDWARE_VERSION };
ATTR_GO_VERSION_RO(version_hardware_tx_dongle, "hardware_version", TX_DONGLE);
...
ATTR_GO_VERSION_RO(version_product_left, "product_version", LEFT_CONTROLLER);

struct go_cfg_attr version_protocol_left = { PROTOCOL_VERSION };
ATTR_GO_VERSION_RO(version_protocol_left, "protocol_version", LEFT_CONTROLLER);

struct go_cfg_attr version_firmware_left = { FIRMWARE_VERSION };
ATTR_GO_VERSION_RO(version_firmware_left, "firmware_version", LEFT_CONTROLLER);

struct go_cfg_attr version_hardware_left = { HARDWARE_VERSION };
ATTR_GO_VERSION_RO(version_hardware_left, "hardware_version", LEFT_CONTROLLER);
...
ATTR_GO_CAL_WO(cal_trigg_left, "calibrate_trigger", SET_TRIGGER_CFG, index,
	       LEFT_CONTROLLER);
...
ATTR_GO_CAL_WO(cal_joy_left, "calibrate_joystick", SET_JOYSTICK_CFG, index,
	       LEFT_CONTROLLER);
static DEVICE_ATTR_RO_NAMED(cal_joy_left_index, "calibrate_joystick_index");

struct go_cfg_attr cal_gyro_left = { GYRO_CALIBRATE };
ATTR_GO_CAL_WO(cal_gyro_left, "calibrate_gyro", SET_GYRO_CFG, index,
	       LEFT_CONTROLLER);
...
ATTR_GO_DEVSTATUS_RO(cal_trigg_left_status, "calibrate_trigger_status",
		     LEFT_CONTROLLER, CALDEV_TRIGGER);

struct go_cfg_attr cal_joy_left_status = { GET_CAL_STATUS };
ATTR_GO_DEVSTATUS_RO(cal_joy_left_status, "calibrate_joystick_status",
		     LEFT_CONTROLLER, CALDEV_JOYSTICK);

struct go_cfg_attr cal_gyro_left_status = { GET_CAL_STATUS };
ATTR_GO_DEVSTATUS_RO(cal_gyro_left_status, "calibrate_gyro_status",
...
ATTR_GO_VERSION_RO(version_product_right, "product_version", RIGHT_CONTROLLER);

struct go_cfg_attr version_protocol_right = { PROTOCOL_VERSION };
ATTR_GO_VERSION_RO(version_protocol_right, "protocol_version",
		   RIGHT_CONTROLLER);

struct go_cfg_attr version_firmware_right = { FIRMWARE_VERSION };
ATTR_GO_VERSION_RO(version_firmware_right, "firmware_version",
		   RIGHT_CONTROLLER);

struct go_cfg_attr version_hardware_right = { HARDWARE_VERSION };
ATTR_GO_VERSION_RO(version_hardware_right, "hardware_version",
		   RIGHT_CONTROLLER);
...
ATTR_GO_CAL_WO(cal_trigg_right, "calibrate_trigger", SET_TRIGGER_CFG, index,
	       RIGHT_CONTROLLER);
...
ATTR_GO_CAL_WO(cal_joy_right, "calibrate_joystick", SET_JOYSTICK_CFG, index,
	       RIGHT_CONTROLLER);
static DEVICE_ATTR_RO_NAMED(cal_joy_right_index, "calibrate_joystick_index");

struct go_cfg_attr cal_gyro_right = { GYRO_CALIBRATE };
ATTR_GO_CAL_WO(cal_gyro_right, "calibrate_gyro", SET_GYRO_CFG, index,
	       RIGHT_CONTROLLER);
...
ATTR_GO_DEVSTATUS_RO(cal_trigg_right_status, "calibrate_trigger_status",
		     RIGHT_CONTROLLER, CALDEV_TRIGGER);

struct go_cfg_attr cal_joy_right_status = { GET_CAL_STATUS };
ATTR_GO_DEVSTATUS_RO(cal_joy_right_status, "calibrate_joystick_status",
		     RIGHT_CONTROLLER, CALDEV_JOYSTICK);

struct go_cfg_attr cal_gyro_right_status = { GET_CAL_STATUS };
ATTR_GO_DEVSTATUS_RO(cal_gyro_right_status, "calibrate_gyro_status",
...
ATTR_GO_FEATURE_RW(rgb_enabled, "enabled", index, UNSPECIFIED);
...
struct led_classdev_mc go_cdev_rgb = {
	.led_cdev = {
		...
		.brightness = 0x50,
		.max_brightness = 0x64,
		.brightness_set = hid_lgo_brightness_set,
	},
...
static void cfg_setup(struct work_struct *work)
{
	...
	ret = mcu_property_out(drvdata.hdev, MCU_CONFIG_DATA, GET_LIGHT_CFG,
...
static int hid_lgo_cfg_probe(struct hid_device *hdev,
...
static void hid_lgo_cfg_remove(struct hid_device *hdev)
{
	guard(mutex)(&drvdata.cfg_mutex);
...
static void hid_lgo_remove(struct hid_device *hdev)
{
...
MODULE_DEVICE_TABLE(hid, hid_lgo_devices);

static struct hid_driver hid_lenovo_go = {
	.name = "hid-lenovo-go",
	.id_table = hid_lgo_devices,
	.probe = hid_lgo_probe,
